# Z1 멀티플레이 아키텍처와 미니 IOCP 서버 구현 계획

## 문서 목적과 역할

이 문서는 현재 싱글플레이 Z1을 Windows Socket 기반 멀티플레이로 확장하기 위한 확정 설계와 단계별 구현 계획을 기록한다. 실제 구현 상태, 검증 결과와 다음 작업은 [Z1_MULTIPLAYER_PROGRESS.md](Z1_MULTIPLAYER_PROGRESS.md)에서 관리한다.

- CraftEngine과 Z1의 실행·데이터 구조
...
설계상 `SocketAPI` 경계의 실제 프로젝트 이름은 `Sockets`이다. 구현 도중 계약이 달라지면 코드와 이 문서를 같은 변경에서 갱신하고, 변경 시점의 상태와 검증 결과는 개발 현황 문서에 남긴다.
...
- 제품 범위를 고가용성 서비스로 확장하는 경우에만 client 재접속 token과 서버 발급 player/session identity를 검토한다. 현재 Z1 MVP와 후속 구현 계획에는 포함하지 않는다.
...
네트워크 Player가 사망해도 `GameOverLevel`로 전환하지 않는다. 서버는 HP 0과 dead 상태를 snapshot에 유지하고 해당 Player의 이동·공격 입력을 무시한다. 클라이언트는 사망 표현을 유지한다. MVP에는 respawn과 자동 재접속을 넣지 않으며 다시 플레이하려면 클라이언트를 재시작해 새 playerId로 입장하거나 서버를 재시작한다. 플레이 흐름상 respawn이 필요해지면 서버가 정한 tick과 spawn 위치로 부활시키는 규칙을 별도로 추가한다.
...
완료 기준: 비정상 종료와 새 클라이언트의 반복 접속이 다른 Session이나 서버 월드에 영향을 주지 않는다. 각 새 연결은 별도의 playerId를 받는다.
