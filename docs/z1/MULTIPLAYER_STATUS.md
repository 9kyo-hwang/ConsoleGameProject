...
TCP payload와 Player 이동 vertical slice에 정적 Enemy 표현이 연결된 상태다. Z1의 실제 입력이 `C2S_Input`으로 서버에 도착하고, 서버가 20Hz Tick에서 BlockingMap 충돌을 통과한 Player 좌표와 수신 Player의 관심 Room Enemy를 `S2C_WorldSnapshot`으로 전송한다. local playerId는 `MyPlayer`, 원격 playerId는 `NetworkPlayer`로 표시한다.
...
다음 작업은 active Room의 Enemy 이동과 Player 상호작용을 서버 권위로 넣는 것이다.

1. Enemy 종류별 또는 최소 공통 이동 정책을 서버 `TickEnemy`에 구현하고, 후보 위치의 BlockingMap·`homeRoom` 경계를 검사한다.
2. 같은 Room의 Player 중 서버가 선택한 대상을 기준으로 Enemy 행동을 결정한다.
3. Player-Enemy 접촉 정책과 피해·사망 상태를 서버에 추가하고 Snapshot에 반영한다.
4. 두 Z1 클라이언트가 같은 Room에서 같은 Enemy 이동·피해·사망 결과를 보는지 확인한다.

이후 순서는 Enemy Projectile과 공격 효과, Session 제거와 안전한 종료다. 완료되지 않은 항목을 구현된 현재 구조처럼 설계 문서에 옮겨 적지 않는다.
